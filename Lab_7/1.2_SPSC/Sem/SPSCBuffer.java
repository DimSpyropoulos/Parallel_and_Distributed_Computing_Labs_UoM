...
//	private Semaphore mutex = new Semaphore(1); // περιττό αφού υπάρχει 1 producer και 1 consumer που ο καθένας τροποποιεί διαφορετικές μεταβλητές
...
//		try {
//			mutex.acquire();  Δεν χρειάζεται γιατί δεν υπάρχει άλλο νήμα να τροποποιεί τη back
//		} catch (InterruptedException e) { }
		back = (back + 1) % size;
		contents[back] = data;
		counter++;
	...
	//	try {
	//		mutex.acquire();  Δεν χρειάζεται γιατί δεν υπάρχει άλλο νήμα να τροποποιεί τη front
	//	} catch (InterruptedException e) { }
		data = contents[front];
